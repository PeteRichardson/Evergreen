...
public final class Logger {
    ...
    */
    ...
    {
        var localwasHandled : Bool = wasHandled
        for handler in handlers {
            handler.emitEvent(event)
            localwasHandled = true
        }
        if let parent = self.parent where shouldPropagate {
            parent.handleEvent(event, wasHandled: localwasHandled)
        } else {
            if !localwasHandled {
...
public enum LogLevel: Int, CustomStringConvertible, Comparable {
    ...
    public init?(description: String) {
        let localdescription :String = description.lowercaseString
        var i = 0
        while let logLevel = LogLevel(rawValue: i) {
            if logLevel.description.lowercaseString == localdescription {
                self = logLevel
                return
            }
            i += 1
